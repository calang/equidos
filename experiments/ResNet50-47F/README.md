...
Train a model for equid identification using transfer learning, learning to identify the 47 individual equids in the dataset using **face pictures only**.
...
This experiment uses data from `data/TunHorseDB2015F` which contains only front face pictures, unlike `data/TunHorseDB2015G` which includes front, left and right face pictures.
...
- `data/THFtraining/` - Training set (70% of images)
- `data/THFvalidation/` - Validation set (15% of images)
- `data/THFtest/` - Test set (15% of images)
...
**Train the model:**
```bash
./experiments/ResNet50-47F/train_model.py
```

**Evaluate on test set:**
```bash
./experiments/ResNet50-47F/test_model.py
# or with custom options:
./experiments/ResNet50-47F/test_model.py --test-dir data/THFtest --output results.csv
```

**Make predictions:**
```bash
./experiments/ResNet50-47F/predict.py <image_path>
```

**Visualize training:**
```bash
./experiments/ResNet50-47F/visualize_training.py --plot
```

**Quick start guide:**
```bash
./experiments/ResNet50-47F/quickstart.py
```
