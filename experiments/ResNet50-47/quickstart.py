...
def main():
    ...
    logger.info("=" * 70)
    logger.info("EQUID IDENTIFICATION MODEL - TRAINING WORKFLOW")
    logger.info("=" * 70)

    # Check source data
    logger.info("\n1. Check Source Data:")
    source_exists = check_directory(
        Path("data/TunHorseDB2015G/Croped Images"),
        "Source images directory"
    )

    # Check prepared data
    logger.info("\n2. Check Prepared Data:")
    train_exists = check_directory(Path("data/THGtraining"), "Training data")
    val_exists = check_directory(Path("data/THGvalidation"), "Validation data")
    test_exists = check_directory(Path("data/THGtest"), "Test data")
